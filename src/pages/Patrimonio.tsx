...
export default function Patrimonio() {
  ...
  const monthlyEvolution = useMemo<MonthData[]>(() => {
    // Collect all months from transactions and sales
  ...
  const currentNetWorth = monthlyEvolution.length > 0
    ? monthlyEvolution[monthlyEvolution.length - 1].accumulated
    : 0;

  const previousNetWorth = monthlyEvolution.length > 1
    ? monthlyEvolution[monthlyEvolution.length - 2].accumulated
    : 0;

  const monthGrowth = currentNetWorth - previousNetWorth;
  const growthPct = previousNetWorth !== 0
    ? ((monthGrowth / Math.abs(previousNetWorth)) * 100)
    : currentNetWorth !== 0 ? 100 : 0;
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="glass rounded-2xl p-4 space-y-2">
              ...
              <p className={`text-lg font-bold tabular-nums ${monthGrowth >= 0 ? 'text-income' : 'text-expense'}`}>
                {formatCurrency(monthGrowth)}
              </p>
            </div>
            <div className="glass rounded-2xl p-4 space-y-2">
              ...
              <p className={`text-lg font-bold tabular-nums ${growthPct >= 0 ? 'text-income' : 'text-expense'}`}>
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <YAxis
                    tick={{ fontSize: 11, fill: 'hsl(210, 10%, 60%)' }}
              ...
              {[...monthlyEvolution].reverse().map(d => (
                <div key={d.month} className="flex items-center justify-between py-2 border-b border-border/30 last:border-0">
                  ...
                  <div className="text-right">
                    <p className={`text-sm font-bold tabular-nums ${d.balance >= 0 ? 'text-income' : 'text-expense'}`}>
